...
export function DashboardPage() {
  ...
  const cards = [
    { label: 'Товары', value: stats?.totalProducts ?? 0, icon: Package },
    { label: 'Заказы', value: stats?.totalOrders ?? 0, icon: ShoppingCart },
    { label: 'Пользователи', value: stats?.totalUsers ?? 0, icon: Users },
    {
      label: 'Выручка',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map(card => (
          <div key={card.label} className="rounded-2xl border border-border bg-muted/30 p-5">
              ...
              <tbody>
                {stats.recentOrders.map((order: any) => (
                  <tr key={order.id} className="border-b border-border/50">
                    ...
                    <td className="py-2">{order.total.toLocaleString('ru-RU')} ₽</td>
                    <td className="py-2">
          ...
          <div className="space-y-3">
            {stats.topProducts.map((tp: any) => (
              <div key={tp.productId} className="flex items-center justify-between text-sm">
                <span className="text-foreground">{tp.product?.name ?? tp.productId}</span>
                <span className="text-muted-foreground">{tp._sum?.quantity ?? 0} шт.</span>
              </div>
            ))}
          </div>
        ) : (
