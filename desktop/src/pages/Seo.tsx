...
const staticPages = [
{ id: 'home', name: 'Главная страница', slug: '/' },
]
...
const statusConfig = {
good: { icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-500/10', label: 'Настроено' },
partial: { icon: AlertCircle, color: 'text-amber-500', bg: 'bg-amber-500/10', label: 'Частично' },
empty: { icon: Circle, color: 'text-muted-foreground/40', bg: 'bg-muted', label: 'Пусто' },
}
...
{/* Status badge */}
<div className={`absolute left-2 top-2 flex items-center gap-1 rounded-full ${st.bg} px-2 py-0.5`}>
...
const previewDesc = form.description || form.ogDescription || 'Описание страницы для поисковых систем...'
...
<div className={`flex h-5 w-9 items-center rounded-full transition-colors ${form.noIndex ? 'bg-red-500' : 'bg-muted'}`}>
<div className={`h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${form.noIndex ? 'translate-x-4.5' : 'translate-x-0.5'}`} />
</div>
<div>
<span className={`text-sm font-medium ${form.noIndex ? 'text-red-500' : 'text-foreground'}`}>
noIndex
</span>
...
const overLimit = maxRecommended && value.length > maxRecommended
...
<span className={`text-[10px] tabular-nums ${overLimit ? 'text-amber-500' : 'text-muted-foreground/60'}`}>
...
rows={2}
className={`flex w-full rounded-lg border bg-background px-3 py-2 text-sm text-foreground resize-none placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary ${
