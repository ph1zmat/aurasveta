...
Конфиг: `vitestconfig.ts`
- Environment: `node`
- Паттерны: `__tests__/**/*.test.ts`, `tests/**/*.test.ts`

Пример существующих тестов (`__tests__/seoandsecuritytest.ts`):
...
5. **Security headers:** X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, Referrer-Policy, Permissions-Policy (в `nextconfig.ts`).
...
- **Prisma 7** — использует `prismaconfig.ts`, адаптер Neon.
