...
- `widgets/header/ui/topbar.tsx` и `widgets/footer/ui/footer.tsx` читают навигацию из БД через `lib/navigation/sitenav.ts`.
- Если в БД нет пунктов для зоны или чтение временно недоступно, используются fallback-ссылки из `shared/config/legacysitenav.ts`.
...
Скрипт: `scripts/migratesitenavitems.ts`
