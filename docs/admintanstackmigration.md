...
2. В `packages/shared-admin/src/utils/searchparams.ts` расширены helpers для чтения enum/boolean params и безопасного merge query-state.
...
1. Добавлен общий `admin` navigation contract в `app/admin/components/adminnav.ts`, который теперь используется и sidebar, и command palette.
...
- `app/admin/webhooks/webhookconfig.ts`
