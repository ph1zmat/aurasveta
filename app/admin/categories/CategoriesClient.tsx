			...
			<div className='flex flex-1 flex-col gap-2 p-3'>
				...
				<div className='flex gap-1.5'>
					{onClick && (
						<button
							onClick={onClick}
							className='flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-muted px-3 py-1.5 text-xs font-medium text-(--nl-accent) transition-colors hover:bg-primary/20'
						>
							Открыть
...
function CategoryDetailView({
	...
	const { data: seoRecord } = trpc.seo.getByTarget.useQuery({
		targetType: 'category',
		targetId: categoryId,
	})
			...
			<div className='overflow-hidden rounded-2xl border border-border bg-muted/10'>
				<div className='flex gap-6 p-6'>
					...
					<div className='w-44 shrink-0'>
						<MediaPicker
							value={category.imagePath ?? null}
							onChange={(key, originalName) => {
								if (key) {
								...
								removeImageMut.mutate(categoryId)
							}}
					...
					<div className='flex flex-1 flex-col gap-3'>
						...
						<div className='flex flex-wrap items-center gap-2'>
							...
							{category.filterSummary && (
								<span className='inline-flex items-center rounded-full bg-muted px-3 py-1 text-xs text-(--nl-accent)'>
									{category.filterSummary}
								</span>
							)}
							{parentCategory ? (
								<button
									...
									className='inline-flex items-center gap-1 rounded-full bg-background px-3 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground'
								>
									<ChevronsRight className='h-3 w-3' />
									Родитель: {parentCategory.name}
								</button>
							) : null}
							{seoRecord ? (
								<span className='inline-flex items-center rounded-full bg-(--nl-success)/10 px-3 py-1 text-xs text-emerald-700 dark:text-emerald-300'>
									SEO override
								</span>
							) : (
