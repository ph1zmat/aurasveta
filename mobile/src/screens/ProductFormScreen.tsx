...
export function ProductFormScreen({ navigation, route }: ProductFormProps) {
  ...
  const { data: existing } = trpc.products.getById.useQuery(editId!, { enabled: !!editId })
  const utils = trpc.useUtils()

  const createMut = trpc.products.create.useMutation({
    onSuccess: () => { utils.products.getMany.invalidate(); navigation.goBack() },
  })
  const updateMut = trpc.products.update.useMutation({
    onSuccess: () => { utils.products.getMany.invalidate(); navigation.goBack() },
  })
  const updateImageMut = trpc.products.updateImagePath.useMutation({
    onSuccess: () => { if (editId) utils.products.getById.invalidate(editId) },
  })
  ...
  useEffect(() => {
    if (existing) {
      ...
      if (existing.propertyValues?.length) {
        setPropertyValues(existing.propertyValues.map((pv: any) => ({
  ...
  const handleSave = async () => {
    ...
    if (!price) { Alert.alert('Ошибка', 'Введите цену'); return }
  ...
  const updateProperty = (index: number, field: 'propertyId' | 'value', val: string) => {
    setPropertyValues(prev => prev.map((pv, i) => i === index ? { ...pv, [field]: val } : pv))
  }
  ...
  const propertyOptions = (properties ?? []).map((p: any) => ({ label: `${p.name} (${p.type})`, value: p.id }))
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {isEdit && (
          <View style={styles.field}>
            ...
            <ImagePicker
              imageUrl={(existing as any)?.images?.[0] ?? null}
        ...
        <View style={styles.row}>
          <Input label="Цена ₽ *" value={price} onChangeText={setPrice} keyboardType="numeric" containerStyle={[styles.field, { flex: 1 }]} />
          <Input label="Старая цена" value={compareAtPrice} onChangeText={setCompareAtPrice} keyboardType="numeric" containerStyle={[styles.field, { flex: 1, marginLeft: spacing.sm }]} />
        </View>

        <View style={styles.row}>
          <Input label="Остаток" value={stock} onChangeText={setStock} keyboardType="numeric" containerStyle={[styles.field, { flex: 1 }]} />
          <Input label="Артикул (SKU)" value={sku} onChangeText={setSku} containerStyle={[styles.field, { flex: 1, marginLeft: spacing.sm }]} />
        </View>

        <Input label="Описание" value={description} onChangeText={setDescription} multiline numberOfLines={4} style={{ height: 100, textAlignVertical: 'top' }} containerStyle={styles.field} />

        <View style={styles.field}>
          <Select label="Категория" options={categoryOptions} value={categoryId} onValueChange={setCategoryId} placeholder="Без категории" />
        </View>

        <View style={styles.row}>
          <Input label="Бренд" value={brand} onChangeText={setBrand} containerStyle={[styles.field, { flex: 1 }]} />
          <Input label="Страна" value={brandCountry} onChangeText={setBrandCountry} containerStyle={[styles.field, { flex: 1, marginLeft: spacing.sm }]} />
        </View>

        {/* Active toggle */}
        <View style={styles.switchRow}>
          <Text style={styles.label}>Активный</Text>
          <Switch value={isActive} onValueChange={setIsActive} trackColor={{ true: colors.primary }} thumbColor={colors.white} />
        </View>
        ...
        <View style={styles.section}>
          ...
          {propertyValues.map((pv, i) => {
            const prop = (properties ?? []).find((p: any) => p.id === pv.propertyId) as any
            return (
              <View key={i} style={styles.propRow}>
                ...
                <Button title="✕" variant="ghost" size="sm" onPress={() => removeProperty(i)} style={{ marginLeft: spacing.xs }} />
              </View>
            )
          })}
          <Button title="+ Добавить свойство" variant="secondary" size="sm" onPress={addProperty} style={{ marginTop: spacing.sm }} />
        </View>

        {/* Save */}
        <Button title={isEdit ? 'Сохранить' : 'Создать товар'} onPress={handleSave} loading={saving} style={{ marginTop: spacing.xl }} />
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: fontSize.base, fontWeight: fontWeight.semibold, color: colors.foreground, marginBottom: spacing.md },
  propRow: { flexDirection: 'row', alignItems: 'center', marginBottom: spacing.sm },
})
